...
public class RecipeToRecipeCommandConverter implements Converter<Recipe, RecipeCommand> {
    ...
    public RecipeCommand convert(Recipe source) {
        ...
        command.setNotes(this.notesToNotesCommandConverter.convert(source.getNotes()));

        if (source.getCategories() != null && source.getCategories().size() > 0) {
            source.getCategories()
                    .forEach((Category category) -> command.getCategories().add(this.categoryToCategoryCommandConverter.convert(category)));
        }

        if (source.getIngredients() != null && source.getIngredients().size() > 0) {
            source.getIngredients()
                    .forEach(ingredient -> command.getIngredients().add(this.ingredientToIngredientCommandConverter.convert(ingredient)));
        }
